...
void chain_print(const ARC::Integrator<Particle> &c, const double ds, const double w, const double pre) {
...
//               <<"    -f :          use constant external force for each particles (read fx, fy, fz after reading each particle data in data file)\n"
               ...
               <<"          --load-particle:         load particle data generated by ParticleList.dump (switched off)\n"
               <<"          --load-par    [char]:    load Parameters parameter dump file with filename as argument (switched off)\n"
...
//           <<"extra-force: "<<fflag<<std::endl
  ...
  // new Integrator class
  ARC::Integrator<Particle> c(n);
...
//      if (fflag) fs>>f[i][0]>>f[i][1]>>f[i][2];
  ...
  // initialization of Integrator system
  ...
  while (true) {
    ...
    if ((tend<0&&i==nstep)||(tend>0&&std::abs(c.getTime()-tend)<terr)) {
      c.dump("Integrator_snapshot_dump");
      pars.dump("Integrator_pars_dump");
      break;
    }
