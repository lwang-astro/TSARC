...
#do
#    echo 'order='${order[i]}
#    ./chain -N 5 input -k ${order[i]} -n 1 1>data.log 2>data.err
#    echo ${order[i]}' '`tail -1 data.log` >>sym_order
#done
...
do
	...
	tail -1 sym_err.k${order[i]}.s${step[j]} |awk -v o=${order[i]} -v s=${step[j]} '{print o,s,$3,$13}' >>sym_prof
    done
done
